...
class MECenv(gym.Env):
    ...
    def __init__(self):
        ...
        self.all_Vehicles, self.all_MECs, self.all_UAVs = HelperInitialize()
    ...
    def update_info(self, vehicle_utility, mec_utility, complete_delay, total_energy, computing_energy, travel_energy
        ...
        self.info['total_energy'].append(sum(total_energy))
    ...
    def step(self, actions):
        ...
        #  link:连接矩阵（前MEC后UAV）
        link, distance, offload_rate, connect_num = UpdateCoverageBelong(self.all_Vehicles, self.all_MECs,
                                                                         self.all_UAVs)
        ...
        for i in range(len(self.all_Vehicles)):
            if offload_severs_list[i] == self.base.all_MECs_num + self.base.all_UAVs_num:
                vehicle_utility[i], complete_delay[i], total_energy[i], complete_delay_normal[i], energy_normal[i] = (
                    LocalOffUtility(self.base, self.all_Vehicles[i], self.all_Tasks[i], self.all_Vehicles[i].avail_CPU,
                                                                                         price_policy[i]))
        ...
        reward = -sum(complete_delay) * 1 + -(
                sum(total_energy) ) * 0.3 + sum(travel_energy) * 0.03 + utility_overboard
        ...
        info['upload_delay'] = sum(upload_delay)  # 总时延
        info['total_energy'] = sum(total_energy)  # 总能耗
        info['travel_energy'] = sum(travel_energy)
