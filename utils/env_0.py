...
# from HelperInitialize import HelperInitialize
# from InitializeMap import InitializeMap
# from InitializeTask import InitializeTask
# from UpdateCoverageBelong import UpdateCoverageBelong
# from LocalOffUtility import LocalOffUtility
# from EdgeOffUtility import EdgeOffUtility
...
# actor 生成是使用字典params
'''MEC:其他类型的MEC,UAV:UAV类型的MEC,Vehicles:相当于用户设备端'''
class MECenv(gym.Env):
    ...
    def __init__(self):
        ...
        self.all_Tasks = InitializeTask(self.base) # 生成所有任务 :contentReference[oaicite:0]{index=0}
        self.timestep = 0
    ...
    def step(self, actions):
        ...
        # ========== 显式构造卸载策略变量 a_{u,·}[n] ==========
        # a_u0[u] = 1 表示用户 u 在当前时隙本地计算zja
        # a_uG[u] = 1 表示用户 u 卸载到 MEC（这里只有 1 个 G）
        # a_um[u, m] = 1 表示用户 u 卸载到第 m 架 UAV
        '''代码里的 “MEC” 和 “UAV” 是 “物理节点分离” 的设定 —— 不是 “MEC 在 UAV 上”，
        而是 “地面有独立的 MEC 服务器（比如你之前说的 MBS，宏基站集成 MEC 功能），
        UAV 是另一类独立的计算节点（可能自带边缘计算能力，也可能只是单纯中继，但代码里按 “UAV 可直接接收卸载任务” 设计）”。'''
        a_u0 = np.zeros(self.base.all_Vehicles_num, dtype=int)
        a_uG = np.zeros(self.base.all_Vehicles_num, dtype=int)
        a_um = np.zeros((self.base.all_Vehicles_num, self.base.all_UAVs_num), dtype=int)

        for u in range(self.base.all_Vehicles_num):
            s = offload_severs_list[u]
            # 本地：索引 = MEC 数 + UAV 数（你原来就用这个 index 表示“本地”）
            if s == self.base.all_MECs_num + self.base.all_UAVs_num:
                a_u0[u] = 1
            # MEC：索引在 [0, all_MECs_num)
            elif 0 <= s < self.base.all_MECs_num:
                a_uG[u] = 1
            # UAV：索引在 [all_MECs_num, all_MECs_num + all_UAVs_num)
            elif self.base.all_MECs_num <= s < self.base.all_MECs_num + self.base.all_UAVs_num:
                m_idx = s - self.base.all_MECs_num   # 第几架 UAV
                a_um[u, m_idx] = 1
            else:
                # 理论上不应该出现
                pass

        # 可以把本时隙的 a 存到 info 里，方便分析和画图
        self.current_a_u0 = a_u0
        self.current_a_uG = a_uG
        self.current_a_um = a_um
        ...
        self.update_info(vehicle_utility, mec_utility, complete_delay, total_energy, computation_energy, travel_energy
                         , complete_delay_normal, energy_normal, upload_delay)
        # 记录轨迹，画图用
        for i in range(len(self.all_Vehicles)):
            self.users_pos[i].append(np.array(self.all_Vehicles[i].position[:2]))
        for i in range(len(self.all_UAVs)):
